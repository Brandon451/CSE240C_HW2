		...
		do	
			...
			then
				...
				do
					...
					echo $param_val
					sed -i "" "s/#define\ $param.*/#define\ $param\ $param_val/g" $policy.cc
					tmp=$(mktemp)
					jq ".executable_name = \"bin/champsim_${policy}_${param}_${param_val}\"" champsim_config.json > "$tmp" && mv "$tmp" champsim_config.json 	
